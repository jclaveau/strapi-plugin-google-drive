...
const HomePage = () => {
  ...
  const generateAuthUrl = ({ client_id, scopeType: _scopeType }) => {
    ...
    const _authUrl = 'https://accounts.google.com/o/oauth2/v2/auth' + '?' + querystring.stringify({
      response_type: 'code',
      client_id: client_id || (currentRow ? currentRow.client_id : ''),
  ...
  return (
    <Container>
      ...
      <Table
        ...
        onClickRow={(e, data) => {
          setCurrentRow(data);
          generateAuthUrl({ client_id: data.client_id});
          setIsModalOpen(true);
        }}
        rows={rows}
        rowLinks={[
        ...
        <Padded top left right bottom size="lg">
          <Select
            ...
            onChange={({ target: { value } }) => {
              setScopeType(value);
              generateAuthUrl({ scopeType: value });
            }}
